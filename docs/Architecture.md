...
`apiRequest()` additionally owns **offline transport routing**: it decides per-call whether to hit the backend or fall back to the Local Printer Agent (`requestLocalFallback`), based on `auth.offlineSession`, `navigator.onLine`, and whether the route supports offline (`supportsOfflineRoute`). This logic is centralized here specifically so no service or store has to know about offline/online branching — see "Offline" below before touching it.
