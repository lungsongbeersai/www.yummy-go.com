...
- **Printing** — `src/services/printer/` abstracts receipt, kitchen slip and payment printing over two transports: browser/agent (Local Printer Agent at `NEXT_PUBLIC_PRINTER_AGENT_URL`, default `http://127.0.0.1:7777`) and native mobile TCP (`@deedarb/capacitor-tcp-socket`). TCP jobs append an ESC/POS real-time status query after every cut, queue the batch continuously, then advance progress once each corresponding printer response arrives.
- **Online-only cutover** — sales, cart, kitchen confirmation, QR, payment and all reads/writes require Backend. The old local sales database, queues, review pages, feature modules and branch toggle have been removed from active source. `providers.tsx` keeps only the one-way browser cleanup needed by previously deployed clients: unregister old service workers, delete Cache Storage, delete the retired Dexie database, and remove retired device/recovery markers. `public/offline-sw.js` remains temporarily as a self-unregistering tombstone so browsers controlled by an older worker can receive the retirement. Printer Agent remains an online USB/Wi-Fi print transport and dedupe boundary; it is not a sales database.
