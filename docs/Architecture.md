...
- **Offline** — `src/lib/offline-routes.ts` is the exact-path allowlist for offline page access. Every established protected menu destination is viewable on desktop and Capacitor: `OFFLINE_READ_ONLY_PATHS` uses Agent SQLite or the scoped Dexie response mirror, `OFFLINE_WRITE_CAPABLE_PATHS` contains screens with reviewed durable workflows, and `OFFLINE_INFRA_PATHS` contains app/account/public shells. The public entry documents (`/`, `/home`, `/policy`, `/login`) and cashier entry documents are warmed during service-worker install, so the marketing/login path does not depend on a prior authenticated session. Page access is platform-identical, but mutation capability is still enforced per endpoint; opening a page does not make its writes offline-safe. This table must stay in lockstep with `OFFLINE_ROUTES`/`OFFLINE_GET_ROUTES` in `src/services/offline-sync.ts`, which is the list of API endpoints that actually support the offline path — the two lists describe the same feature from the UI side and the API side. `network-store.ts` starts at `CHECKING` and probes `/api/v1/sync/health`; any HTTP response proves reachability, while only repeated response-less transport failures can enter `OFFLINE`. Browser/Electron network events trigger probes but never set state directly — with the superseding exceptions recorded in `Decisions.md`; `navigator.onLine === true` behaviour is unchanged. `AuthGuard` redirects only unknown/non-warmed paths while `auth-store.offlineSession` is true; `useAppShellData` overlays `MenuItem.offlineLocked` from the same list so web and Capacitor navigation agree. `OfflineConnectivityDialog` (mounted in `providers.tsx`) shows once per confirmed disconnect episode. The service worker (`src/service-worker/sw.ts`, compiled by `@serwist/next` into `public/offline-sw.js`) explicitly bypasses `/api/*` and `/app-version.json` so it never competes with this Dexie/agent-based caching. `src/proxy.ts` permanently canonicalizes `www.yummy-go.com` to `yummy-go.com`, because service-worker registrations and caches cannot be shared between those origins.
