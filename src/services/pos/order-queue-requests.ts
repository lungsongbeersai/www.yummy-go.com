...
export async function fetchOrderQueue(params: FetchOrderQueueParams) {
  ...
  try {
    const response = await apiClient.get<FetchOrderQueueResponse>("/api/v1/pos/customer_order_queue", {
      params: {
...
export const sendToKitchen = (input: SendToKitchenInput) => {
  requiredItems(input.order_uuids, "order_uuids");

  return apiRequest<SendToKitchenResponse>("patch", "/api/v1/pos/customer_order_queue/send_to_kitchen", {
    data: {
      order_uuids: input.order_uuids,
...
// bulk เวอร์ชันของ confirmOrderItemServed (services/pos/requests.ts) ที่ตะกร้า POS ใช้
// อยู่แล้ว — endpoint เดียวกันแต่ตะกร้าเรียกทีละ order_it_uuid ส่วนคิวออเดอร์ต้องยิง
// order_item_uuids เป็น array ทีเดียวหลายแถว จึงแยกฟังก์ชันไว้ ไม่แก้ตัวเดิม
export const confirmOrderItemsServed = (input: ConfirmOrderItemsServedInput) => {
  requiredItems(input.order_item_uuids, "order_item_uuids");

  return apiRequest<{ status: string; message: string }>("patch", "/api/v1/pos/confirm_order_item_served", {
    data: {
