...
export function useZzProbeWorkflow({
  initialTableUuid,
  initialTableName,
}: ZzInput) {
  ...
  // ดึงค่าที่ callback ใช้จริงออกมาเป็นตัวแปรเดี่ยว เพราะถ้าอ่าน user.branch_uuid ข้างใน callback
  // React Compiler จะอนุมาน dependency เป็น user ทั้งก้อน ซึ่งไม่ตรงกับ dependency ที่ประกาศไว้
  ...
  // effect โหลดเมนูยิงตั้งแต่ render แรกเมื่อมีสาขา จึงเริ่มที่สถานะกำลังโหลดเลย
  // (เดิม effect เป็นคนเซ็ต true ทำให้เห็นเมนูว่างแวบหนึ่งก่อน skeleton จะขึ้น)
  const [loadingMenu, setLoadingMenu] = useState(() => Boolean(branchUuid));
  ...
  // ส่วน async ล้วนของการโหลดเมนู — ไม่มี setState แบบ synchronous จึงให้ effect เรียกตรง ๆ ได้
  // โดยไม่เกิด cascading render (การตั้งธง loading ย้ายไปไว้ที่ผู้เรียกและที่ render-time reset)
  const runMenuLoad = useCallback(
    async ({
      cateUuid = "",
      query = "",
  ...
  // ตัวที่ event handler เรียก (เลือกหมวด/ค้นหา/refresh) — ตั้งธง loading ทันทีก่อนยิง request
  // ทำได้เพราะอยู่ใน event handler ไม่ใช่ใน effect
  const loadMenu = useCallback(
    async (options: LoadMenuOptions = {}) => {
      if (!branchUuid) {
        setCategories([]);
      ...
      setLoadingMenu(true);
      await runMenuLoad(options);
    },
    [branchUuid, runMenuLoad],
  );
  ...
  // สลับโต๊ะแล้วต้องปิดแผงตะกร้าของโต๊ะเดิม — เป็นการปรับ state ตาม prop จึงทำตอน render
  useResetOnDeps([initialTableUuid, initialTableName], () => {
  ...
  // ส่วน "ปรับ state ทันที" ของการโหลดเมนูรอบใหม่ ย้ายมาทำตอน render โดยผูกกับ runMenuLoad
  // ซึ่งเป็น dependency เดียวกับ effect ด้านล่าง จังหวะจึงตรงกับของเดิม
  useResetOnDeps([runMenuLoad], () => {
    if (!branchUuid) {
      setCategories([]);
      setMenuBySort(emptyMenuBySort());
      setLoadingMenu(false);
  ...
  useEffect(() => {
    void Promise.resolve().then(() => runMenuLoad({ refreshCategories: true }));
  }, [runMenuLoad]);

  // ไม่มีผู้ใช้ = ไม่มี printer context — เคลียร์ตอน render ส่วน effect เหลือแต่งาน async
  // ไม่เคลียร์ตอนเปลี่ยนภาษา เพื่อคงพฤติกรรมเดิมที่ค่าเก่ายังใช้ได้จนกว่าค่าใหม่จะมา
  useResetOnChange(userUuid, () => {
    if (userUuid) return;
  ...
  }, [language, resolvePrinterDeviceContext, userUuid]);
...
export type ZzProbe = ReturnType<typeof useZzProbeWorkflow>;
