...
export interface VatPrintRow {
  invoice: string;
  grandTotal: number;
}

export interface VatPrintData {
  branchName: string;
  cashier: string;
  dateFrom: string;
  dateTo: string;
  grandTotal: number;
  labels: VatPrintLabels;
  rows: VatPrintRow[];
  storeName: string;
}

// รายงานนี้มีไว้ยื่นภาษี — ต้องพิมพ์ครบทุกใบเสร็จตามที่แสดงบนหน้าจอ ไม่จำกัด Top N (ต่างจากรายงาน
// สรุปอื่นที่ตัดรายละเอียดออกได้ เพราะที่นี่แต่ละแถวคือหลักฐานใบเสร็จหนึ่งใบ ตัดไม่ได้)
export function buildVatPrintData({
  dateFrom,
  dateTo,
  labels,
  rows,
  ...
  return {
    ...
    labels,
    rows: rows.map((row) => ({ invoice: row.order_invoice, grandTotal: row.grand_total })),
...
export function renderVatPrintHtml(data: VatPrintData) {
  ...
  const bodyHtml = `
    ...
    ${dividerHtml}
    ${listHeaderHtml}
    ${rows.map((r) => row(r.invoice, r.grandTotal)).join("") || `<p style="text-align:center">-</p>`}
    ${dividerHtml}
...
// เวอร์ชันพิมพ์ผ่าน printer agent — ขนาดตัวอักษร/โครงสร้างยอดรวม/ตัวแบ่งตรงกับ payment-methods ทุกจุด
export function buildVatReportOps(data: VatPrintData): ReportPrintOp[] {
  const { labels, rows } = data;
  ...
  return [
    ...
    divider,
    ...rows.map((row) => lr(row.invoice, row.grandTotal)),
    divider,
