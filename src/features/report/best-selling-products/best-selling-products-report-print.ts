...
// จึงตัด subtotal/discount/service/vat/ราคาต่อหน่วยออก เหลือแค่อันดับ + จำนวนขาย + ยอดขายต่อสินค้า
// แบ่งเป็นหมวดตามกลุ่มสินค้าเหมือนบนหน้าจอ (ไม่จำกัด Top N — สอดคล้องกับ daily-sales ที่ไม่จำกัดจำนวนรายการ
// ต่อกลุ่มเช่นกัน) เลขลำดับเริ่มนับ 1 ใหม่ทุกกลุ่ม ตรงกับที่แสดงบนตาราง (ไม่ใช้ item.rank ซึ่งเป็นลำดับรวมทั้งรายงาน)
  ...
  const groupsHtml = groups
    ...
    <section class="category">
      ...
      ${group.items
        .map(
          (item) => `
        ${row(`${item.rank}. ${item.name} (${item.qty})`, item.finalTotal)}
