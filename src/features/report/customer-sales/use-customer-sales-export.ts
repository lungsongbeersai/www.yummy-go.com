...
import type { CustomerSalesResponse } from "@/services/report";
...
import {
  buildCustomerSalesPrintData,
  buildCustomerSalesReportOps,
  renderCustomerSalesPrintHtml,
} from "./customer-sales-print";
import {
  customerSalesFileBaseName,
  customerSalesRowId,
  customerSalesSummaryFromRows,
  customerSalesSummaryRows,
  customerSalesTableSection,
  type CustomerSalesExportData,
} from "./customer-sales-excel";

// รายงานนี้โหลดข้อมูลทั้งช่วงวันที่ในครั้งเดียวอยู่แล้ว (API ไม่รองรับ page/limit — ดู
// customer-sales-page.tsx) จึงไม่ใช้ useStandardReportWorkflow เหมือน employee-sales/vat —
// fetchExportData ใช้ current.customer_reports/summary ที่อยู่ใน memory แล้วโดยตรง ไม่ยิง API ซ้ำ
export function useCustomerSalesExport({
  branchLabel,
  current,
  dateFrom,
  dateTo,
  ...
  branchLabel: string;
  current: CustomerSalesResponse | null;
  dateFrom: string;
  dateTo: string;
  ...
  // เลือกแถวไว้ก่อน export Excel/PDF = ส่งออกเฉพาะลูกค้าที่เลือก พร้อมสรุปยอดใหม่ตามแถวที่เลือก
  // (พิมพ์ผ่าน printer agent ไม่ผูกกับการเลือกแถว — พิมพ์ทั้งรายงานเสมอ ตรงกับรายงานอื่น)
  async function fetchExportData(): Promise<CustomerSalesExportData> {
    if (!current) throw new Error(t("report.branchRequired"));
    if (!selectedCount) return { reportName: reportTitle, rows: current.customer_reports, summary: current.summary };

    const rows = current.customer_reports.filter((row) => selectedRowIds.has(customerSalesRowId(row)));
    return { reportName: reportTitle, rows, summary: customerSalesSummaryFromRows(rows) };
  }

  const exportActions = useReportExportActions<CustomerSalesExportData>({
    disabled: loading || !current?.customer_reports.length,
    exportReportRef,
    fetchExportData,
    fileBaseName: () => customerSalesFileBaseName({ dateFrom, dateTo, orderBy }),
      ...
      createSingleSheetReportWorkbook(
        ...
        [
          ...
          },
          {
            ...
            rows: customerSalesSummaryRows(data.summary, t),
          },
          customerSalesTableSection(data, t),
        ],
  ...
  async function printReport() {
    ...
    async function fallbackToBrowserPrint(
      printData: ReturnType<typeof buildCustomerSalesPrintData>,
      ...
      renderReceiptPrintWindow(targetWindow, renderCustomerSalesPrintHtml(printData));
    ...
    try {
      const printLabels = {
        ...
        itemsHeaderLeft: t("report.customerSales.customer"),
      ...
      const data = buildCustomerSalesPrintData({
        dateFrom,
        dateTo,
        labels: printLabels,
        rows: current.customer_reports,
      ...
      try {
        ...
        const response = await submitReportPrint({
          ...
          print_document: {
            ...
            copies: 1,
            cut_mode: "per_ticket",
            ops: buildCustomerSalesReportOps(data),
            browser_payload: { title: printLabels.title, html: renderCustomerSalesPrintHtml(data) },
          },
        });
